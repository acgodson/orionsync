    ...
    const getWalletInfo = async () => {
      if (spaceWallet) {
        ...
        if (ethereum) {
          ...
          const share = await contract.shares(address);
          const balance = await contract.balances(address);
          const transactionHistory: any = await contract.getTransactionHistory(
            address
          );
          setWalletInfo({ share, balance });
...
export function useCreateSpace(
  ...
  const createSpace = async () => {
    const participantAddresses = newParticipants.map(
      (participant) => participant.address
    );
    const participantShares = newParticipants.map((participant) =>
      parseInt(participant.share, 10)
    );
    const ethereum = (window as any).ethereum;
    if (!ethereum) {
      return;
