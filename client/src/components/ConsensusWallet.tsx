import { handleOpenExplorer, shortenAddress } from "@/data/constants";
import { useSignAgreement, useSpaceInfo, useWalletInfo } from "@/hooks/wallet";
import {
  Box,
  Button,
  Divider,
...
const WalletCard = ({
  ...
  return (
    ...
    >
      ...
      >
        <Box>
          <Text fontSize="20px" textAlign="left" color="white">
            {title}
          </Text>
          <Text
            fontSize="xs"
...
const ConsensusWallet = ({ spaceId }: { spaceId: string }) => {
  ...
  const membersList: string[] = [
    "0xFAbCB60b5E25e5B09087BBec88C2f79ABC42979C",
    "0xf2750684eB187fF9f82e2F980f6233707eF5768a",
  ];
  const { spaceCreated, walletCreated } = useSpaceInfo(spaceId);
  const { spaceWallet, totalBalance, walletInfo, withdraw, history } =
    useWalletInfo(spaceId, address!);
  const { signatureStatuses, isUserSigned, signAgreement } = useSignAgreement(
    spaceId,
    membersList,
    address!
  );
  const [ethToUsd, setEthToUsd] = useState(null);
  const ethValueInUsd1 = ethToUsd
    ? (
        parseFloat(ethers.utils.formatEther(walletInfo.balance.toString())) *
        ethToUsd
      ).toFixed(3)
    : "-";
      ...
      >
        <HStack spacing={28}>
          <WalletCard
            ...
            amount={totalBalance}
            subtitle={
              walletCreated && spaceWallet ? shortenAddress(spaceWallet) : ""
            }
            symbol="GoerliETH"
          ...
          <WalletCard
            ...
            amount={
              walletInfo
                ? ethers.utils.formatEther(walletInfo.balance.toString())
                : "0"
            }
            symbol="GoerliETH"
        ...
        <VStack w="100%" alignItems="space-between">
          <Text textAlign="left" fontSize="16" color="#545454">
            Transaction History
          </Text>
              ...
              {history &&
                history.map((transaction: any, index: number) => (
                  <Tr key={index}>
                    <Td
                      color={
                        transaction.method === "Deposit" ? "#2BCC7E" : "#ED7770"
                      }
                    >
                      {transaction.method}
                    </Td>
        ...
        {!spaceCreated && (
          <Box w="100%" top={0} p={8}>
            <CreateSpaceForm spaceId={spaceId} />
          </Box>
        )}
      </VStack>
    </HStack>
